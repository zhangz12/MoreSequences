...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(125.0, 55.0),  # All numbers are approximate.
                rg.Point(112.5, 76.7),
                rg.Point(87.5, 76.7),
                rg.Point(75.0, 55.0),
                rg.Point(87.5, 33.3),
                rg.Point(112.5, 33.3)]
    circle = rg.Circle(rg.Point(100, 55), 25)
    answer = generate_points_on_circle(circle, 6)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    list1 = generate_points_on_circle(circle, number_of_points)
    for k in range(len(list1)):
        circles = rg.Circle(list1[k], 10)
        circles.fill_color = color
        circles.attach_to(window)
        dots = list1[k]
        points = rg.Point(dots.x, dots.y)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    list2 = generate_points_on_circle(circle,number_of_slices)
    for k in range(len(list2)):
        slices = rg.Line(circle.center, list2[k])
        slices.color = color
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    list3 = generate_points_on_circle(circle, number_of_segments)
    for k in range(len(list3)):
        if k < len(list3) - 1:
            slices = rg.Line(list3[k], list3[k + 1])
        else:
            slices = rg.Line(list3[len(list3) - 1], list3[0])
        slices.color = color
        slices.thickness = thickness
        slices.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON:  20 segments, hops = 5.'
    window = rg.RoseWindow(480, 350, title)
    ...
    title = 'FANCY POLYGON:  51 segments, hops = 25.'
    window = rg.RoseWindow(480, 350, title)
    ...
    title = 'FANCY POLYGON:  300 segments, hops = 61.'
    window = rg.RoseWindow(480, 350, title)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    circle.attach_to(window)
    list4 = generate_points_on_circle(circle, number_of_lines)
    for k in range(len(list4)):
        if k + hops_to_next_point <= len(list4) - 1:
            slices = rg.Line(list4[k], list4[k + hops_to_next_point])
        else:
            slices = rg.Line(list4[k], list4[k - len(list4) + hops_to_next_point])
        slices.color = color
        slices.thickness = thickness
        slices.attach_to(window)
    window.render()
